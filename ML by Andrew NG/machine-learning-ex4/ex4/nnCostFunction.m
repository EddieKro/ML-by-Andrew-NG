...
err = (lambda/(2*m)) * (sum(sum(regTheta1.^2)) + sum(sum(regTheta2.^2)));
J+=err;
...
Theta1_grad = del1/m + (lambda/m)*[zeros(size(Theta1,1),1) regTheta1];
Theta2_grad  =del2/m + (lambda/m)*[zeros(size(Theta2,1),1) regTheta2];
